...
# BW_CUTOFF  = 0.05     # Butterworth SOH filter cutoff frequency
...
# # Per cell, apply rolling mean after Butterworth
# for cell_id, grp in index_df.groupby("cell_id"):
#     grp_sorted = grp.sort_values("charge_num")
#     smoothed   = pd.Series(grp_sorted["soh_smooth"].values)
#     rolled     = smoothed.rolling(
#                      window=ROLLING_WINDOW,
#                      center=True,
#                      min_periods=1
#                  ).mean().values
#     index_df.loc[grp_sorted.index, "soh_smooth"] = rolled
# # ══════════════════════════════════════════════════════════════════════════
...
Dataset:
    6 LCO pouch cells (CS2_33, CS2_34, CS2_35, CS2_36, CS2_37, CS2_38)
    CC-CV charge at 0.5C to 4.2V, discharge at 1C to 2.7V
    Room temperature (~25 degrees C), no temperature sensor
    ~1000 cycles per cell

SOH Smoothing (two-stage):
    Stage 1: Butterworth low-pass filter (order=3, cutoff=0.05)
             Removes high-frequency cycle-to-cycle noise
    Stage 2: Centred rolling mean (window=10 cycles)
             Removes remaining medium-frequency oscillations
             center=True avoids lag in SOH estimates
    Combined: clean smooth degradation curve preserving
    the real three-phase degradation shape

Reads  : data/CS2_33/*.xlsx  (sheet: Channel_1-006)
         data/CS2_34/*.xlsx  (sheet: Channel_1-007)
         data/CS2_35/*.xlsx  (sheet: Channel_1-008)
         data/CS2_36/*.xlsx  (sheet: Channel_1-009)
         data/CS2_37/*.xlsx  (sheet: Channel_1-010)
         data/CS2_38/*.xlsx  (sheet: Channel_1-011)

Outputs (saved in processed/):
    calce_curves.npy         (N, 101) float32
    calce_soh.npy            (N,)     float32
    calce_temperature.npy    (N,)     float32
    calce_cell_ids.npy       (N,)     str
    calce_index.csv          (N, 9)
...
T_DEFAULT      = 25.0
SOH_MIN        = 0.75
BW_CUTOFF      = 0.02    # reduced from 0.10 -- more aggressive smoothing
BW_ORDER       = 3
ROLLING_WINDOW = 10      # centred rolling mean window size
...
def rolling_smooth(data, window=ROLLING_WINDOW):
    """
    Centred rolling mean.
    center=True  : window centred on each point (no lag introduced)
    min_periods=1: handles edges where full window not available
    """
...
def load_cell(cell_id, sheet_name):
    ...
    full_df = pd.concat(combined, ignore_index=True)
    return full_df.sort_values("Date_Time").reset_index(drop=True)
...
for cell_id, sheet_name in CS2_CELLS.items():
    ...
    cap_per_cycle = extract_capacity_per_cycle(full_df)
    if not cap_per_cycle:
        print(f"  No valid discharge capacities -- skipping")
        continue

    q_max = max(cap_per_cycle.values())
    print(f"  Q_max = {q_max:.4f} Ah")

    cell_curves, cell_records, charge_num = [], [], 0
    ...
    for cyc_idx in sorted(full_df["Cycle_Index"].unique()):
        ...
        cell_curves.append(curve)
        cell_records.append({
            "cell_id": cell_id, "group": "CALCE",
            "charge_num": charge_num, "cyc_idx": cyc_idx,
            "soh": soh, "q_ah": q_ah, "q_max": q_max, "t_mean": t_mean,
        })
...
index_df   = index_df[keep_mask].reset_index(drop=True)
curves_arr = curves_arr[keep_mask.values]
print(f"  Removed  : {n_removed}  Retained : {len(index_df)}")
...
median_curve = np.median(curves_arr, axis=0)
rmse         = np.sqrt(((curves_arr - median_curve)**2).mean(axis=1))
threshold    = rmse.mean() + 3.0 * rmse.std()
outlier_mask = rmse > threshold
outlier_info = list(zip(
    index_df["cell_id"][outlier_mask].tolist(),
    index_df["charge_num"][outlier_mask].tolist()
...
print(f"  Threshold={threshold:.5f}  "
      f"Outliers removed: {outlier_mask.sum()}  "
      f"(first 5: {outlier_info[:5]})")
curves_arr = curves_arr[~outlier_mask]
index_df   = index_df[~outlier_mask].reset_index(drop=True)
print(f"  Retained : {len(index_df)}")

# ══════════════════════════════════════════════════════════════════════════
# STEP 7: TWO-STAGE SOH SMOOTHING PER CELL
# ══════════════════════════════════════════════════════════════════════════
print(f"\nTwo-stage SOH smoothing per cell:")
print(f"  Stage 1 -- Butterworth (order={BW_ORDER}, cutoff={BW_CUTOFF})")
print(f"  Stage 2 -- Centred rolling mean (window={ROLLING_WINDOW})")
...
for cell_id, grp in index_df.groupby("cell_id"):
    ...
    # Stage 1: Butterworth
    stage1 = butterworth_smooth(grp_sorted["soh"].values)

    # Stage 2: Centred rolling mean
    stage2 = rolling_smooth(stage1)
    stage2 = np.clip(stage2, 0.0, 1.0)

    index_df.loc[grp_sorted.index, "soh_smooth"] = stage2

index_df["soh"] = index_df["soh_smooth"]

print(f"\n  Per-cell after smoothing:")
for cell_id, grp in index_df.groupby("cell_id"):
    raw_std    = float(grp["soh_raw"].std())
    smooth_std = float(grp["soh_smooth"].std())
    print(f"    {cell_id}: {len(grp):4d} cycles  "
          f"raw {grp['soh_raw'].min():.4f}-{grp['soh_raw'].max():.4f}  "
          f"smooth {grp['soh_smooth'].min():.4f}-"
          f"{grp['soh_smooth'].max():.4f}  "
          f"noise_std: {raw_std:.4f} -> {smooth_std:.4f}")
...
# ══════════════════════════════════════════════════════════════════════════
print(f"\nValidation:")
assert len(curves_arr) == len(soh_arr) == \
       len(temp_arr)   == len(ids_arr), "Array length mismatch!"
print(f"  All arrays length   : {len(soh_arr)}  -- OK")
print(f"  curves_arr shape    : {curves_arr.shape}")
print(f"  soh_arr shape       : {soh_arr.shape}")
print(f"  V range             : {curves_arr.min():.4f} - "
      f"{curves_arr.max():.4f} V")
...
# ══════════════════════════════════════════════════════════════════════════
cell_colors = {
    "CS2_33": "purple",  "CS2_34": "brown",
    "CS2_35": "steelblue", "CS2_36": "darkorange",
    "CS2_37": "green",   "CS2_38": "crimson",
}
...
axes[0].axhline(0.80, color="red",   lw=1.5, linestyle="--", label="EOL 80%")
axes[0].axhline(0.75, color="black", lw=0.8, linestyle=":", label="75%")
axes[0].set_xlabel("Cycle"); axes[0].set_ylabel("SOH")
axes[0].set_title("SOH: raw (faint) vs two-stage smoothed (solid)")
axes[0].legend(fontsize=8); axes[0].grid(True, alpha=0.4)
axes[0].set_ylim(0.70, 1.05)

common_x, norm_trajs = np.linspace(0, 1, 300), []
for cell_id, grp in index_df.groupby("cell_id"):
    grp    = grp.sort_values("charge_num")
    x_norm = (grp["charge_num"].values - 1) / \
              max(grp["charge_num"].max() - 1, 1)
    f      = np.interp(common_x, x_norm, grp["soh_smooth"].values)
    norm_trajs.append(f)
    axes[1].plot(common_x, f, lw=1.5, alpha=0.7,
                 color=cell_colors[cell_id], label=cell_id)
...
std_traj  = np.std(norm_trajs,  axis=0)
axes[1].plot(common_x, mean_traj, "k-", lw=2.5, label="Mean", zorder=5)
axes[1].fill_between(common_x, mean_traj-std_traj,
                     mean_traj+std_traj,
                     alpha=0.15, color="black", label="+/-1 std")
axes[1].axhline(0.80, color="red", lw=1.5, linestyle="--", label="EOL 80%")
axes[1].set_xlabel("Normalised lifecycle")
axes[1].set_ylabel("SOH")
axes[1].set_title("Normalised Lifecycle")
axes[1].legend(fontsize=8); axes[1].grid(True, alpha=0.4)
axes[1].set_ylim(0.70, 1.05)

plt.suptitle(f"CALCE CS2 -- Two-Stage SOH Smoothing\n"
             f"Butterworth (order={BW_ORDER}, cutoff={BW_CUTOFF}) + "
             f"Rolling mean (window={ROLLING_WINDOW}, centred)",
             fontsize=12, fontweight="bold")
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "calce_soh_final.png"),
            dpi=150, bbox_inches="tight")
...
for plot_idx, (cell_id, grp) in enumerate(index_df.groupby("cell_id")):
    ...
    for i, idx in enumerate(c_idx):
        ax.plot(x_axis, curves_arr[idx],
                color=cmap(norm_c(soh_vals[i])), alpha=0.3, lw=0.5)
    n = len(c_idx)
    ax.plot(x_axis, curves_arr[c_idx[:n//4]].mean(0), "g-", lw=2.5,
            label=f"Early (SOH~{soh_vals[:n//4].mean():.3f})")
    ax.plot(x_axis, curves_arr[c_idx[-n//4:]].mean(0), "r-", lw=2.5,
            label=f"Late  (SOH~{soh_vals[-n//4:].mean():.3f})")
    ax.set_title(f"{cell_id}  n={n}  "
                 f"SOH:{soh_vals.min():.3f}-{soh_vals.max():.3f}",
                 fontsize=9)
    ax.set_xlabel("<- End    Start ->", fontsize=8)
    ax.set_ylabel("V", fontsize=8)
    ax.legend(fontsize=7); ax.grid(True)

for j in range(len(CS2_CELLS), len(axes)):
    axes[j].set_visible(False)

plt.suptitle("CALCE CS2 -- Charge Curves per Cell",
             fontsize=12, fontweight="bold")
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "calce_soh_overview.png"),
            dpi=150, bbox_inches="tight")
